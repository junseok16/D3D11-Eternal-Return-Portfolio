...
HRESULT Client::CItemButton::PreInitialize()
{
	// 델리게이트를 추가합니다.
...
HRESULT Client::CItemButton::Initialize()
{
	m_wpItemInfoPanel = CUIManager::GetInstance()->GetItemInfoPanel();
	...
	case Client::EItemBelongType::CRAFT:
	{
		...
		{
		case Client::EItemSlotType::FIRST:	SetPivot({ 832.0f, 609.0f });	break;
		case Client::EItemSlotType::SECOND:	SetPivot({ 879.0f, 609.0f });	break;
		case Client::EItemSlotType::THIRD:	SetPivot({ 926.0f, 609.0f });	break;
		case Client::EItemSlotType::FOURTH:	SetPivot({ 973.0f, 609.0f });	break;
		default: CHECKF(false, L"Item slot type is not designated.");		break;
		}
	}
	break;

	case Client::EItemBelongType::EQUIPMENT:
	{
		SetWidth(24.0f);
		SetHeight(22.0f);
		...
		switch (m_eItemSlotType)
		{
		case Client::EItemSlotType::FIRST:	SetPivot({ 333.0f, 663.0f });	break;
		case Client::EItemSlotType::SECOND:	SetPivot({ 373.0f, 663.0f });	break;
		case Client::EItemSlotType::THIRD:	SetPivot({ 413.0f, 663.0f });	break;
		case Client::EItemSlotType::FOURTH:	SetPivot({ 333.0f, 692.0f });	break;
		case Client::EItemSlotType::FIFTH:	SetPivot({ 373.0f, 692.0f });	break;
		case Client::EItemSlotType::SIXTH:	SetPivot({ 413.0f, 692.0f });	break;
		default: CHECKF(false, L"Item slot type is not designated.");		break;
		}
	}
	break;
	...
	case Client::EItemBelongType::INVENTORY:
	{
		switch (m_eItemSlotType)
		{
		case Client::EItemSlotType::FIRST:	SetPivot({ 785.0f, 652.0f });	break;
		case Client::EItemSlotType::SECOND:	SetPivot({ 832.0f, 652.0f });	break;
		case Client::EItemSlotType::THIRD:	SetPivot({ 879.0f, 652.0f });	break;
		case Client::EItemSlotType::FOURTH:	SetPivot({ 926.0f, 652.0f });	break;
		case Client::EItemSlotType::FIFTH:	SetPivot({ 973.0f, 652.0f });	break;
		case Client::EItemSlotType::SIXTH:	SetPivot({ 785.0f, 685.0f });	break;
		case Client::EItemSlotType::SEVENTH:SetPivot({ 832.0f, 685.0f });	break;
		case Client::EItemSlotType::EIGHTH:	SetPivot({ 879.0f, 685.0f });	break;
		case Client::EItemSlotType::NINETH:	SetPivot({ 926.0f, 685.0f });	break;
		case Client::EItemSlotType::TENTH:	SetPivot({ 973.0f, 685.0f });	break;
		default: CHECKF(false, L"Item slot type is not designated.");		break;
		}
	}
	break;
	...
	case Client::EItemBelongType::ITEM_BOX:
	{
		switch (m_eItemSlotType)
		{
		case Client::EItemSlotType::FIRST:	SetPivot({ 784.0f, 319.0f });	break;
		case Client::EItemSlotType::SECOND:	SetPivot({ 836.0f, 319.0f });	break;
		case Client::EItemSlotType::THIRD:	SetPivot({ 888.0f, 319.0f });	break;
		case Client::EItemSlotType::FOURTH:	SetPivot({ 940.0f, 319.0f });	break;
		case Client::EItemSlotType::FIFTH:	SetPivot({ 784.0f, 355.0f });	break;
		case Client::EItemSlotType::SIXTH:	SetPivot({ 836.0f, 355.0f });	break;
		case Client::EItemSlotType::SEVENTH:SetPivot({ 888.0f, 355.0f });	break;
		case Client::EItemSlotType::EIGHTH:	SetPivot({ 940.0f, 355.0f });	break;
		default: CHECKF(false, L"Item slot type is not designated.");		break;
		}
	}
	break;
...
HRESULT Client::CItemButton::PostInitialize()
{
...
HRESULT Client::CItemButton::BeginPlay()
{
...
int32 Client::CItemButton::PreTick(float _fDeltaSeconds)
{
...
int32 Client::CItemButton::Tick(float _fDeltaSeconds)
{
	...
	if (m_eItemBelongType == EItemBelongType::INVENTORY)
	{
		if (m_eButtonMouseState == EButtonMouseState::HOVER)
		{
			if (!m_wpItem.expired())
			{
				m_wpItemInfoPanel.lock()->RegisterItem(m_wpItem.lock(), m_eItemSlotType);
			}
		}
		else
		{
			m_wpItemInfoPanel.lock()->UnregisterItem(m_eItemSlotType);
		}
	}
...
int32 Client::CItemButton::PostTick(float _fDeltaSeconds)
{
...
HRESULT Client::CItemButton::EndPlay()
{
...
void Client::CItemButton::Render()
{
	CButton::Render();
}

void Client::CItemButton::Release()
{
	CButton::Release();
}

void Client::CItemButton::OnButtonUpIn()
{
	m_wpSurvivor = m_wpGameManager.lock()->GetSurvivor();
	m_wpInventory = m_wpSurvivor.lock()->GetInventory();
	...
	case Client::EItemBelongType::CRAFT:
	{
		...
		case EItemCategoryType::FOOD:
		case EItemCategoryType::DRINK:
			m_wpSoundComponent.lock()->PlaySoundEx("chracter_Craft_Food", ESoundType::UI_01);
		...
		case EItemCategoryType::MATERIAL:
		case EItemCategoryType::WEAPON:
			m_wpSoundComponent.lock()->PlaySoundEx("chracter_Craft_Tool", ESoundType::UI_01);
	...
	case Client::EItemBelongType::INVENTORY:
	{
		...
		case EItemCategoryType::FOOD:
		{
			...
			m_wpSoundComponent.lock()->PlaySoundEx("Consume_Eat_" + strIndex, ESoundType::UI_01);
		...
		{
			string strIndex = to_string(CMathUtility::GetRandomInteger(1, 3));
			m_wpSoundComponent.lock()->PlaySoundEx("Consume_Drink_" + strIndex, ESoundType::UI_01);
			m_wpInventory.lock()->EatItem((int32)m_eItemSlotType);
		}
		break;

		case EItemCategoryType::HEAD:
			m_wpSoundComponent.lock()->PlaySoundEx("equipmentInstall", ESoundType::UI_01);
	...
	case Client::EItemBelongType::WILD_ANIMAL:
	{
...
void Client::CItemButton::OnButtonUpOut()
{
	m_wpSurvivor = m_wpGameManager.lock()->GetSurvivor();
	m_wpInventory = m_wpSurvivor.lock()->GetInventory();
	...
	case Client::EItemBelongType::INVENTORY:
	{
		...
		m_wpSoundComponent.lock()->PlaySoundEx("DropItem", ESoundType::UI_01);
